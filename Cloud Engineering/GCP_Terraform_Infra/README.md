...
This project uses **Infrastructure as Code (IaC)** to provision a scalable architecture on Google Cloud Platform.
...
2. **Plan (Preview Changes)**:
   ```bash
   terraform plan -var="project_id=YOUR_PROJECT_ID" -var="db_password=securepass"
   ```

3. **Apply (Create Infrastructure)**:
   ```bash
   terraform apply -var="project_id=YOUR_PROJECT_ID" -var="db_password=securepass"
   ```
